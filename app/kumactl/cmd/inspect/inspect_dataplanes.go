	...
	data := printers.Table{
		...
		NextRow: func() func() []string {
			...
			return func() []string {
				...
				if dataplaneInsight.GetMTSL().GetCertificateExpirationTime() != nil {
					certExpiration = util_proto.MustTimestampFromProto(dataplaneInsight.GetMTSL().GetCertificateExpirationTime())
				}
				var lastCertGeneration *time.Time
				if dataplaneInsight.GetMTSL().GetLastCertificateRegeneration() != nil {
					lastCertGeneration = util_proto.MustTimestampFromProto(dataplaneInsight.GetMTSL().GetLastCertificateRegeneration())
				}
				dataplaneInsight.GetMTSL().GetCertificateExpirationTime()
				certRegenerations := strconv.Itoa(int(dataplaneInsight.GetMTSL().GetCertificateRegenerations()))
