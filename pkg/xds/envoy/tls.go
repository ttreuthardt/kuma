...
// It secures inbound listener with certificate of "dp:service" that will be received from the SDS.
...
func CreateDownstreamTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, service string) (*envoy_auth.DownstreamTlsContext, error) {
	...
	commonTlsContext, err := createCommonTlsContext(ctx, metadata, []string{service}, validationSANMatcher)
	if err != nil {
		return nil, err
	}
...
// Server exposes for the the clients all certificates of downstreamServices, which means that if DP has inbound with services "web" and "web-api" and communicates with "backend"
// the client ("backend") will see that DP is connections with TLS certificates of "web" and "web-api".
// There is no way to correlate incoming request to "web" or "web-api" with outgoing request to "backend" to expose only one certificate.
func CreateUpstreamTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, downstreamServices []string, upstreamService string) (*envoy_auth.UpstreamTlsContext, error) {
	if !ctx.Mesh.Resource.MTLSEnabled() {
		return nil, nil
	}
	validationSANMatcher := ServiceSpiffeIDMatcher(ctx.Mesh.Resource.Meta.GetName(), upstreamService)
	commonTlsContext, err := createCommonTlsContext(ctx, metadata, downstreamServices, validationSANMatcher)
	if err != nil {
		return nil, err
	}
...
func createCommonTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, services []string, validationSANMatcher *envoy_type_matcher.StringMatcher) (*envoy_auth.CommonTlsContext, error) {
